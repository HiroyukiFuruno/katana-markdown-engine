use katana_markdown_engine::{KmeDocument, KmeError, TargetResolutionKind};
use serde::Serialize;
...
impl HarnessDocument {
    pub fn new(
        path: PathBuf,
        source: String,
        document: KmeDocument,
        metadata: katana_markdown_engine::MetadataReconcileResult,
    ) -> Self {
        Self {
...
impl From<&katana_markdown_engine::KmeNode> for HarnessNode {
    fn from(node: &katana_markdown_engine::KmeNode) -> Self {
        Self {
            id: node.id.0.clone(),
