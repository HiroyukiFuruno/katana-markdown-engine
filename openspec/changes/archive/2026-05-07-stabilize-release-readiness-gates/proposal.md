...
KMEは `v0.1.0` 公開前に、通常検査とrelease前検査の境界を固定する必要がある。

KMLでは、local targetだけではなくGitHub Actionsとbranch protectionまで含めてrelease前検査を扱っている。KMEでも同じ考え方を採用する。

ただしKMEはlibrary-onlyなので、KMLの多チャネル配布検査は持ち込まない。

## What Changes

- `release-check` をKMEのrelease前検査入口として定義する。
