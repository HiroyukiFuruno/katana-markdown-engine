...
impl MarkdownInput {
    ...
    pub(crate) fn into_parts(self) -> Result<(PathBuf, String), KmeError> {
        match self {
    ...
    fn read_file(path: &Path) -> Result<(PathBuf, String), KmeError> {
        let content = std::fs::read_to_string(path).map_err(|source| KmeError::ReadFile {
