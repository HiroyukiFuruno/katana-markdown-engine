...
KMEのcanonical fixtureは、KatanA現行挙動をKME内で再現するための正本である。

標準テストは `/Users/hiroyuki_furuno/works/private/katana` の絶対パスへ依存しない。KatanA側の現行fixtureを確認したうえで、KME repository内の `tests/fixtures/canonical/` に同期したファイルをテスト入力にする。
...
KatanA側fixtureの更新をKMEへ取り込む場合は、差分を確認してから同期する。
