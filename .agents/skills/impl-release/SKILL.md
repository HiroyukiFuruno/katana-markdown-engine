---
name: impl-release
description: katana-markdown-engine で指定バージョンのOpenSpec実装、release準備、release PR、GitHub Release、crates.io公開、事後整理までを進めるときに使う。/impl-release vX.Y.Z 相当のKME向けリリース実装ワークフロー。
---

# impl-release

KMEの `vX.Y.Z` リリースを、実装から公開後整理まで進める入口です。

## 前提

- 対象repositoryは `/Users/hiroyuki_furuno/works/private/katana-markdown-engine`。
- default branchは `master`。
- KMEはlibrary-onlyです。CLI、UI、binary artifact、npm、PyPI、Homebrew、MCPB、editor extensionのrelease処理を持ち込みません。
- 公開対象は `katana-markdown-engine` crate、GitHub Release、`docs/release-notes/<version>.md` です。
...
- KMEにCLI、描画、export、UI framework、downstream repository依存を追加しそうになった。
...
- 描画、export、scroll、selection、highlight制御をKMEへ入れない。
...
```bash
cd /Users/hiroyuki_furuno/works/private/katana-markdown-engine
gh release view vX.Y.Z --repo HiroyukiFuruno/katana-markdown-engine
cargo search katana-markdown-engine --limit 1
cargo info katana-markdown-engine@X.Y.Z
```
